        ...
        for disp_i, ch_i in enumerate(range(first_channel_to_plot, first_channel_to_plot+n_channels_to_plot)):
            ...
            if self.model.plot_HFOs:
                for i in range(len(hfo_starts)):
                    self.view.plot_waveform(windows_in_time[i], eeg_data_to_display[ch_i, hfo_starts[i]-start_in_time*self.model.sample_freq:hfo_ends[i]-start_in_time*self.model.sample_freq]-disp_i*offset_value, colors[i], 2)
