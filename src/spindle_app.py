...
class SpindleApp(object):
    ...
    def load_edf(self, file_path):
        ...
        self.raw = load_mne_raw(file_path)
    ...
    def filter_eeg_data(self, param_filter: ParamFilter = None):
        ...
        self.filter_data_60 = None
        self.filter_data_60 = self.ensure_filter_data_60()
    ...
    def export_app(self, path):
        ...
        checkpoint.update({
            ...
            "spike_annotations": np.array(self.event_features.spike_annotations) if self.event_features else np.array([]),
    ...
    def capture_current_run(self):
        ...
        run = DetectionRun.create(
            ...
            selected_channels=self.event_features.channel_names if self.event_features is not None else self.channel_names,
            param_filter=self.param_filter,
            param_detector=self.param_detector,
            param_classifier=self.param_classifier,
            event_features=self.event_features,
            detector_output=self.Spindles,
    ...
    def sync_active_run(self):
        ...
        run.detector_output = self.Spindles
        run.classified = self.classified
        run.selected_channels = list(np.array(self.event_features.channel_names if self.event_features is not None else self.channel_names).tolist())
        run.refresh_summary()
    ...
    def activate_run(self, run_id):
        ...
        self.Spindles = run.detector_output
    ...
    def load_checkpoint(self, checkpoint):
        ...
        self.analysis_session = AnalysisSession(self.biomarker_type)
        if self.detected:
            self.Spindles = checkpoint_array(checkpoint, "Spindles")
